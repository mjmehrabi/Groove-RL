 ...
 */
public class HeuIDAstarExploreAction extends SimulatorAction {
    /** Constructs a new action, for a given simulator. */
    public HeuIDAstarExploreAction(Simulator simulator, boolean animated) {
     ...
     */
    public Exploration explore(ExploreType exploreType,ExploringItemIDA exploringItems) {
        return explore(getSimulatorModel().getState(), exploreType,exploringItems);
    }
     ...
     * failed for some reason
     */
    public Exploration explore(GraphState state, ExploreType exploreType,ExploringItemIDA exploringItems) {
        ...
        if(!exploringItems.callFromHeuGenerator){
            ...
            try {
                ...
                ExploreThread generateThread = new ExploreThread(result,exploringItems);
                // go!
                ...
                if(exploringItems.heuristicResult==null && !exploringItems.ModelCheckingTarget.equals("DeadLock"))
            ...
            try {
                SwingUtilities.invokeAndWait(new Runnable() {
                    ...
                    public void run() {
                        ...
                        dialog.setBound(HeuIDAstarExploreAction.this.bound);
                        if (dialog.ask(getFrame())) {
                            HeuIDAstarExploreAction.this.bound = dialog.getBound();
                        } else {
                            setInterrupted(true);
                        }
                    }
    ...
    private class ExploreThread extends Thread {
         ...
         */
        public ExploreThread(Exploration exploration,ExploringItemIDA exploringItems) {
        	if(!exploringItems.callFromHeuGenerator){
        ...
        final public void run() {
        	if(!exploringItems.callFromHeuGenerator) {
                ...
                this.exploration.heuristicIDAplay(gts, state, exploringItems);
                if(exploringItems.isProgressVisible){
        		...
        		setInterrupted(false);
        		this.exploration.heuristicIDAplay(exploringItems.gts, exploringItems.initialState, exploringItems);
        	}
        ...
        private ExploringItemIDA exploringItems;
    }
