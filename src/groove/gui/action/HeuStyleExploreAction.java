 ...
 */
public class HeuStyleExploreAction extends SimulatorAction {
    /** Constructs a new action, for a given simulator. */
    public HeuStyleExploreAction(Simulator simulator, boolean animated) {
     ...
     */
    public Exploration explore(ExploreType exploreType,ArrayList<LearningItem> ALearningItems,String ModelCheckingType,String ModelCheckingTarget,boolean isFirstStep) {
        return explore(getSimulatorModel().getState(), exploreType,ALearningItems,ModelCheckingType,ModelCheckingTarget,isFirstStep);
    }
     ...
     * failed for some reason
     */
    public Exploration explore(GraphState state, ExploreType exploreType,ArrayList<LearningItem> ALearningItems,String ModelCheckingType,String ModelCheckingTarget,boolean isFirstStep) {
        ...
        SimulatorModel simModel = getSimulatorModel();
            ...
            try {
                ...
                ExploreThread generateThread = new ExploreThread(result,ALearningItems, ModelCheckingType, ModelCheckingTarget, isFirstStep);
                // go!
    ...
    public Exploration exploreSmallModel(GraphState state, ExploreType exploreType) {
    	Exploration result=null;
		try {
			result = new Exploration(exploreType, state);
            ...
            try {
                SwingUtilities.invokeAndWait(new Runnable() {
                    ...
                    public void run() {
                        ...
                        dialog.setBound(HeuStyleExploreAction.this.bound);
                        if (dialog.ask(getFrame())) {
                            HeuStyleExploreAction.this.bound = dialog.getBound();
                        } else {
                            setInterrupted(true);
                        }
                    }
    ...
    private class ExploreThread extends Thread {
         ...
         */
        public ExploreThread(Exploration exploration,ArrayList<LearningItem> ALearningItems,String ModelCheckingType,String ModelCheckingTarget,boolean isFirstStep) {
               this.cancelDialog = createCancelDialog();
               this.progressListener =createProgressListener();
               this.exploration = exploration;
               this.ALearningItems=ALearningItems;
               this.isFirstStep=isFirstStep;
               this.ModelCheckingType=ModelCheckingType;
               this.ModelCheckingTarget=ModelCheckingTarget;
        }
        ...
        final public void run() {
            ...
            this.exploration.heuristicLEplay(gts, state, ALearningItems, ModelCheckingType, ModelCheckingTarget,isFirstStep);
            if(ALearningItems.get(0).isProgressVisible){
        ...
        ArrayList<LearningItem> ALearningItems;
        String ModelCheckingType;
        String ModelCheckingTarget;
